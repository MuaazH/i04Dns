...
//	str := f.name + " [" + strings.Join(*(f.words), " ") + "]"
	str := f.name
	if f.isShadowed {
		str = str + " [S]"
	}
...
func newNameFilter(name string) *NameFilter {
	...
	obj := NameFilter {&words, low, false }
	return &obj
}

func (list *FilterList) Add(name string) *FilterList {
    l := len(*list.names)
	item := newNameFilter(name)
    if l == cap(*list.names) {
        sz := l * 2
        if sz < 2 {
            sz = 2
        }
        arr := make([]*NameFilter, sz)
        slc := arr[0:l+1]
        copy(slc, *list.names)
        slc[l] = item
		list.names = &slc
...
func (list *FilterList) Remove(index int) *FilterList {
	...
	if size > 0 && index >= 0 && index < size {
		...
		if last < quarter && quarter > 2 {
			arr := make([]*NameFilter, list.Capacity() >> 1)
			for i := 0; i < index; i++ {
				arr[i] = (*list.names)[i]
			}
			j := index + 1
...
func verifyFilter(matcher *NameFilter, x *NameFilter, wg *sync.WaitGroup) {
	defer wg.Done()
...
func matches(text string, f *NameFilter) bool {
	L := len(*(f.words))
	idx := 0
	var success bool
	for i := 0; i < L; i++ {
...
func compare(a *NameFilter, b *NameFilter) int {
	// ASCII Only!!
	if a == nil {
		if b == nil {
			return 0
...
func mergeSort(array *[]*NameFilter) {
	size := len(*array)
	temp := make([]*NameFilter, size)
	mergeSortDoSort(array, &temp, 0, size - 1)
}

func mergeSortDoSort(a *[]*NameFilter, aux *[]*NameFilter, lo int, hi int) {
	if hi <= lo {
		return
	}
	mid := (lo + hi) >> 1
...
func mergeSortDoMerge(a *[]*NameFilter, aux *[]*NameFilter, lo int, mid int, hi int) {
    // copy to aux[]
    for k := lo; k <= hi; k++ {
        (*aux)[k] = (*a)[k] 
    }
