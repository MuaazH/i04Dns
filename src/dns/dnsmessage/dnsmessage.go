...
func IsNameEqual(a *Name, b *Name) bool {
	if a == nil {
		if b == nil {
			return true
		}
...
func IsBodyEqual(a *Resource, b *Resource) bool {
	if a == nil {
		if b == nil {
			return true
		}
	...
	case TypeA:
		...
		return true
	case TypeAAAA:
		x := a.Body.(*AAAAResource)
		y := b.Body.(*AAAAResource)
		for i := 0; i < 16; i++ {
			if x.AAAA[i] != y.AAAA[i] {
				return false
			}
		}
		return true
	case TypeCNAME:
		x := a.Body.(*CNAMEResource)
		y := b.Body.(*CNAMEResource)
		return IsNameEqual(&x.CNAME, &y.CNAME)
	case TypeMX:
		x := a.Body.(*MXResource)
		y := b.Body.(*MXResource)
		return x.Pref == y.Pref && IsNameEqual(&x.MX, &y.MX)
	case TypeNS:
		x := a.Body.(*NSResource)
		y := b.Body.(*NSResource)
		return IsNameEqual(&x.NS, &y.NS)
	case TypePTR:
		x := a.Body.(*PTRResource)
		y := b.Body.(*PTRResource)
		return IsNameEqual(&x.PTR, &y.PTR)
	case TypeSOA:
		x := a.Body.(*SOAResource)
		y := b.Body.(*SOAResource)
		return IsNameEqual(&x.NS, &y.NS) && IsNameEqual(&x.MBox, &y.MBox) && x.Serial == y.Serial
	case TypeTXT:
		x := a.Body.(*TXTResource)
		y := b.Body.(*TXTResource)
		if x.TXT == nil {
			if y.TXT == nil {
				return true
			}
		...
		return true
	case TypeSRV:
		x := a.Body.(*SRVResource)
		y := b.Body.(*SRVResource)
		return IsNameEqual(&x.Target, &y.Target) && x.Port == y.Port
	}
	return false
}
